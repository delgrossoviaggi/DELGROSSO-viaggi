...
type Payload = {
  booking?: {
    id?: string; codice?: string; nome?: string; cognome?: string;
    telefono?: string; email?: string; posti?: number;
    posti_selezionati?: string; totale?: number;
  };
  trip?: {
    id?: string; titolo?: string; destinazione?: string;
    data_partenza?: string; ora_partenza?: string; luogo_partenza?: string;
  };
  pdfBase64?: string;
  pdfFilename?: string;
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!booking?.email || !String(booking.email).includes("@")) {
      return json({ success: false, error: "Email cliente mancante o non valida." }, 400);
    }
    if (!body.pdfBase64) {
      return json({ success: false, error: "PDF della ricevuta mancante." }, 400);
    }
    ...
    if (!host || !username || !password) {
      return json({ success: false, error: "Configurazione SMTP della Edge Function incompleta." }, 500);
    }
    ...
    const text = [
      ...
      "",
      "In allegato trovi la ricevuta della prenotazione con il tuo QR Code da presentare al check-in.",
      "",
    ...
    const html = `
      <div style="font-family:Arial,Helvetica,sans-serif;line-height:1.6;color:#24324a">
        ...
        <p>In allegato trovi la ricevuta della prenotazione con il tuo
        <strong>QR Code</strong> da presentare al check-in.</p>
        ...
        <p>Grazie,<br><strong>Del Grosso Viaggi &amp; Limousine Bus</strong></p>
      </div>`;
