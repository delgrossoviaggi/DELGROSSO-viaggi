...
async function restArchive(){
  ...
  if(r.ok) {
    ...
    const nb=await fetch(`${SUPABASE_URL}/rest/v1/noleggi_bus_pagamenti?select=*,noleggi_bus(id_noleggio,referente,azienda,email,tratta_partenza,tratta_destinazione)&receipt_path=not.is.null&order=receipt_generated_at.desc&limit=2000`,{headers}).then(x=>x.ok?x.json():[]).catch(()=>[]);
    const extra=(Array.isArray(nb)?nb:[]).map(x=>({tipo_documento:String(x.tipo||'').toLowerCase()==='saldo'?'saldo':'acconto',documento_id:x.id,pagamento_id:x.id,prenotazione_id:null,cliente:x.noleggi_bus?.referente||x.noleggi_bus?.azienda||'Cliente',viaggio:`${x.noleggi_bus?.tratta_partenza||'—'} → ${x.noleggi_bus?.tratta_destinazione||'—'}`,email:x.noleggi_bus?.email||null,data_documento:x.receipt_generated_at||x.data_pagamento,importo:x.importo,storage_path:x.receipt_path,email_inviata:x.receipt_email_sent,id_noleggio:x.noleggi_bus?.id_noleggio||null,noleggio_pagamento:true}));
    return [...base,...extra];
  }
  ...
  const [pRes,gRes,nbRes]=await Promise.all([
    ...
    fetch(`${SUPABASE_URL}/rest/v1/noleggi_bus_pagamenti?select=*,noleggi_bus(id_noleggio,referente,azienda,email,tratta_partenza,tratta_destinazione)&receipt_path=not.is.null&order=receipt_generated_at.desc&limit=2000`,{headers})
  ]);
  const p=await pRes.json().catch(()=>[]), g=await gRes.json().catch(()=>[]), nb=await nbRes.json().catch(()=>[]);
  ...
  const noleggioDocs=(Array.isArray(nb)?nb:[]).map(x=>({tipo_documento:String(x.tipo||'').toLowerCase()==='saldo'?'saldo':'acconto',documento_id:x.id,numero_documento:x.receipt_number,pagamento_id:x.id,prenotazione_id:null,cliente:x.noleggi_bus?.referente||x.noleggi_bus?.azienda||'Cliente',viaggio:`${x.noleggi_bus?.tratta_partenza||'—'} → ${x.noleggi_bus?.tratta_destinazione||'—'}`,email:x.noleggi_bus?.email||null,data_documento:x.receipt_generated_at||x.data_pagamento,importo:x.importo,storage_path:x.receipt_path,email_inviata:x.receipt_email_sent,id_noleggio:x.noleggi_bus?.id_noleggio||null,noleggio_pagamento:true}));
