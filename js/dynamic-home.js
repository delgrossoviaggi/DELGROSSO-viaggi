...
(()=>{
 ...
 function render(){
  ...
  grid.innerHTML=rows.length?rows.map(t=>{
   ...
   const a=document.createElement('a');a.className='trip-question';const contact=document.querySelector('a[data-setting="whatsapp"][href]')?.href||'https://wa.me/393205730466';a.href=contact.split('?')[0]+'?text='+encodeURIComponent(`Buongiorno, vorrei informazioni su ${tripTitle(t)} del ${fmtDate(t.data_partenza)}.`);a.textContent='Hai una domanda? Scrivici su WhatsApp';box.querySelector('.trip-body')?.append(a);
