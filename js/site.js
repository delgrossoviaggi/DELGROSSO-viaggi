...
async function home(){
 ...
 const fg=document.querySelector('#fleetGrid');if(fg)fg.innerHTML=publicFleetItems(f.data).length?publicFleetItems(f.data).map(x=>fleetCard(x,true)).join(''):`<div class="empty">Flotta in aggiornamento.</div>`;
 ...
 try{applySettings(await loadSettings())}catch(e){console.warn('Contatti non aggiornati',e)}
}
...
function startTripDetailLive(id){
 ...
 const refresh=async()=>{try{const list=await getGestionaleTrips(),t=list.find(x=>String(x.id)===String(id));if(!t)return;const a=gestTripAvailability(t),status=document.querySelector('#tripLiveStatus'),side=document.querySelector('#tripSideFree'),bar=document.querySelector('#tripSideProgress'),seat=document.querySelector('#tripLiveSeats');if(status){status.classList.toggle('sold',a.soldOut);const strong=status.querySelector('strong'),sp=status.querySelector('span');if(strong)strong.textContent=a.soldOut?'SOLD OUT':a.free<=10?'ULTIMI POSTI':'DISPONIBILE';if(sp)sp.textContent=a.soldOut?'Nessun posto libero':`${a.free} posti ancora disponibili`;const sm=status.querySelector('small');if(sm)sm.textContent=tripCountdown(t.data_partenza,String(t.ora_partenza||'').slice(0,5));}if(side)side.textContent=a.soldOut?'SOLD OUT':a.free;if(bar)bar.style.width=`${a.total?Math.round(((a.total-a.free)/a.total)*100):0}%`;if(seat){const strong=seat.querySelector('strong'),span=seat.querySelector('span');if(strong)strong.textContent=a.soldOut?'0':a.free;if(span)span.textContent=`disponibili su ${a.total||'—'}`;}const bookingUrl='prenota.html?viaggio='+encodeURIComponent(id);const ctas=[['.detail-top-actions a','Prenota →','Altre partenze →'],['.trip-detail-hero-actions a:first-child','Scegli il tuo posto →','Viaggio al completo →'],['.detail-full-btn','Prenota ora →','Scopri altre partenze →'],['.detail-final-cta a','Vai alla prenotazione →','Esplora partenze →']];ctas.forEach(([selector,availableLabel,fullLabel])=>{const link=document.querySelector(selector);if(!link)return;link.href=a.soldOut?'viaggi.html':bookingUrl;link.textContent=a.soldOut?fullLabel:availableLabel;link.classList.toggle('btn-soldout',a.soldOut);});}catch(e){console.warn('Aggiornamento dettaglio non riuscito',e)}};
