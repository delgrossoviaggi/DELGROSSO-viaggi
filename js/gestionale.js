...
async function createGestionaleBooking({viaggioId,nome,cognome,telefono,email,note,posti}){
  const result = await gestBridge('booking',{viaggioId,nome,cognome,telefono,email:(email||null),note,posti:posti.map(String)});
  // La prenotazione è già stata registrata: la ricevuta PDF è un passaggio
  // successivo e non deve mai trasformare una prenotazione valida in errore.
  try{
    if(typeof window.generatePrenotazioneRicevuta === 'function'){
      const trips = await getGestionaleTrips();
      const trip = trips.find(v => String(v.id) === String(viaggioId));
      await window.generatePrenotazioneRicevuta({
        booking:{
          id:result?.id_prenotazione || result?.id || result?.numero || '',
          numero:result?.numero || '',
          nome,cognome,telefono,
          posti:Array.isArray(result?.posti) && result.posti.length ? result.posti : posti
        },
        trip
      });
    }
  }catch(receiptError){
    console.warn('Ricevuta PDF non generata automaticamente:',receiptError);
  }
  return result;
}
