...
const CONFIG = Object.freeze({
  ...
  retryBaseMs: 1500,
  maxRetries: 8,
  queueKey: 'dg_supabase_sync_queue_v1',
  stateKey: 'dg_supabase_sync_state_v1',
  heartbeatMs: 30000,
  flushMs: 5000,
  realtime: true,
});
...
function isTransientError(error) {
  ...
  if (!code) return /network|fetch|failed to fetch|timeout|aborted/i.test(String(error?.message || error || ''));
