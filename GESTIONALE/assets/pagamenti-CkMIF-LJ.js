import"./settingsService-SYNC153.js";import{t as e}from"./tripService-BzTorehO.js";import{t}from"./bookingService-CitenMQF.js";import{r as n}from"./notificationService-D35-IFKU.js";import{t as r}from"./appRoutes-BbuDm13X.js";/* empty css                      */import"./backButton-BaCuvMXq.js";import{n as i,t as a}from"./messageSystem-jVMshBDs.js";import{a as o,c as s,i as c,o as l,s as ee}from"./notificationCenterService-CaZQlods.js";import"./brandShell-CIWNUkWr.js";import{t as te}from"./xlsxExport-Ci5qKKp3.js";import{issuePaymentReceipt as DGIssuePaymentReceipt}from"./paymentReceiptService-v24.js";import*as DGDocs from"./bookingDocumentsService-v25.js";var paymentPickerModal=document.getElementById(`paymentPickerModal`),paymentBookingSearch=document.getElementById(`paymentBookingSearch`),paymentBookingPickerList=document.getElementById(`paymentBookingPickerList`),btnNewPayment=document.getElementById(`btnNewPayment`),closePaymentPicker=document.getElementById(`closePaymentPicker`),pickerType=`Acconto`,u=document.querySelector(`#paymentsTable tbody`),ne=document.getElementById(`searchPayment`),re=document.getElementById(`filterMetodo`),ie=document.getElementById(`filterStato`),ae=document.getElementById(`btnRefresh`),d=document.getElementById(`btnPending`),oe=document.getElementById(`btnQuickExcel`),se=document.getElementById(`btnExportIncassi`),ce=document.getElementById(`btnOpenBookings`),f=document.getElementById(`todayIncome`),p=document.getElementById(`pendingIncome`),m=document.getElementById(`depositIncome`),h=document.getElementById(`balanceIncome`),g=document.getElementById(`refundIncome`),_=document.getElementById(`modal`),v=document.getElementById(`paymentModalTitle`),y=document.getElementById(`paymentBookingMeta`),cl=document.getElementById(`paymentLinkedClient`),tr=document.getElementById(`paymentLinkedTrip`),bk=document.getElementById(`paymentLinkedBooking`),selNote=document.getElementById(`paymentSelectionNote`),changeBooking=document.getElementById(`changePaymentBooking`),ci=document.getElementById(`paymentClienteId`),vi=document.getElementById(`paymentViaggioId`),b=document.getElementById(`modalPayTotale`),x=document.getElementById(`modalPayPagato`),S=document.getElementById(`modalPayResiduo`),C=document.getElementById(`modalPayStatus`),w=document.getElementById(`paymentHistoryTbody`),T=document.getElementById(`bookingIdInput`),E=document.getElementById(`movementIdInput`),D=document.getElementById(`importoInput`),O=document.getElementById(`tipoSelect`),k=document.getElementById(`metodoSelect`),A=document.getElementById(`dataInput`),j=document.getElementById(`noteInput`),M=document.getElementById(`savePayment`),le=document.getElementById(`closeModal`),N=document.getElementById(`cancelPaymentEdit`),ue=document.getElementById(`btnQuickAcconto`),de=document.getElementById(`btnQuickSaldo`),fe=document.getElementById(`btnQuickRimborso`),P=[],F=[],I=[],L=[],R=!1,z=``,B=null,V=null,H=null,pe=!1,saveBusy=!1;function U(e,t=`info`){i({type:t===`error`?`error`:`info`,title:t===`error`?`Errore`:`Pagamenti`,message:String(e||``)})}function W(e){let t=Number(e);return Number.isFinite(t)?t:0}function G(e){return new Intl.NumberFormat(`it-IT`,{style:`currency`,currency:`EUR`}).format(W(e))}function K(e){if(!e)return`-`;let t=new Date(e);return Number.isNaN(t.getTime())?`-`:t.toLocaleDateString(`it-IT`)}function q(e=``){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function me(e){let t=String(e?.viaggio_id||e?.tratta_id||``),n=F.find(e=>String(e.id)===t);return n?.titolo||n?.destinazione||e?.viaggio_nome||e?.viaggio||e?.viaggio_codice||n?.codice||t||`Viaggio`}function he(e){return e?.data||e?.data_prenotazione||e?.created_at||null}function ge(e){return String(e||``).trim().toLowerCase()===`annullata`}function _e(e){let t=String(e?.message||e||``).toLowerCase();return t.includes(`modulo pagamenti non disponibile`)||t.includes(`public.pagamenti`)}function ve(){let e=new Map;I.forEach(t=>{let n=String(t?.prenotazione_id||``);n&&(e.has(n)||e.set(n,[]),e.get(n).push(t))}),L=P.filter(e=>!ge(e?.stato)).map(t=>{let n=e.get(String(t.id||``))||[];return{booking:t,payments:n,summary:o(t,n),tripLabel:me(t)}}).sort((e,t)=>new Date(he(t.booking)||0).getTime()-new Date(he(e.booking)||0).getTime())}function ye(e){return e===c.paid?`status-paid`:e===c.partial?`status-partial`:e===c.refunded?`status-refunded`:`status-pending`}function be(e){return e===`Saldo`?`movement-badge movement-badge--saldo`:e===`Rimborso`?`movement-badge movement-badge--refund`:e===`Acconto`?`movement-badge movement-badge--deposit`:`movement-badge movement-badge--other`}function J(){let e=String(ne?.value||``).trim().toLowerCase(),t=String(re?.value||``).trim().toLowerCase(),n=String(ie?.value||``).trim();return L.filter(r=>{let i=`${r.booking?.codice||``} ${r.booking?.cliente_nome||r.booking?.cliente||``} ${r.tripLabel} ${r.summary.latestMethod||``}`.toLowerCase();return!(e&&!i.includes(e)||t&&String(r.summary.latestMethod||``).toLowerCase()!==t||n&&r.summary.status!==n||R&&r.summary.residual<=0)})}function xe(e){let{booking:t,summary:n,tripLabel:r}=e,i=n.latestMovement?.tipo||`Nessun movimento`,a=n.lastPaymentAt?K(n.lastPaymentAt):`-`;return`
  ...
  `}function Se(){let e=new Date().toISOString().slice(0,10),t=I.filter(t=>String(t.data_pagamento||``).slice(0,10)===e).reduce((e,t)=>e+ee(t),0),n=L.reduce((e,t)=>e+t.summary.residual,0),r=I.filter(e=>e.tipo===`Acconto`).reduce((e,t)=>e+l(t),0),i=I.filter(e=>e.tipo===`Saldo`).reduce((e,t)=>e+l(t),0),a=I.filter(e=>e.tipo===`Rimborso`).reduce((e,t)=>e+l(t),0);f&&(f.textContent=G(t)),p&&(p.textContent=G(n)),m&&(m.textContent=G(r)),h&&(h.textContent=G(i)),g&&(g.textContent=G(a))}function Y(){E&&(E.value=``),D&&(D.value=``),O&&(O.value=`Acconto`),k&&(k.value=`Contanti`),A&&(A.value=new Date().toISOString().slice(0,10)),j&&(j.value=``),M&&(M.textContent=`Salva movimento`),N?.classList.add(`hidden`)}function X(e,t){if(D){if(e===`Saldo`){D.value=t.residual>0?t.residual.toFixed(2):``;return}e===`Rimborso`&&(D.value=t.paidNet>0?t.paidNet.toFixed(2):``)}}function Ce(e){
  const amount=ee(e), sign=amount<0?`-`:`+`;
  const receipt=e.receipt_storage_path
    ? `<button type="button" class="btn-secondary" data-dg-receipt-path="${q(e.receipt_storage_path)}">Ricevuta PDF</button><button type="button" class="btn-secondary" data-dg-doc-action="payment-email" data-payment-id="${q(e.id)}">📧</button><button type="button" class="btn-secondary" data-dg-doc-action="payment-wa" data-payment-id="${q(e.id)}">🟢</button>`
    : `<button type="button" class="btn-secondary dg-receipt-retry" data-payment-id="${q(e.id)}">📄 Genera ricevuta</button>`;
  return `
    <tr data-movement-id="${e.id}">
      <td>${K(e.data_pagamento||e.created_at)}</td>
      <td>${q(e.ricevuta||e.receipt_number||`-`)}</td>
      <td><span class="${be(e.tipo)}">${q(e.tipo||`Acconto`)}</span></td>
      <td class="${amount<0?`amount-negative`:`amount-positive`}">${sign}${G(Math.abs(amount))}</td>
...
function bookingPickerText(e){
  const b=e.booking||e;
  return `${b.codice||``} ${b.cliente_nome||b.cliente||``} ${b.telefono||b.cliente_telefono||``} ${e.tripLabel||me(b)} ${b.email||b.cliente_email||``}`.toLowerCase();
}
function bookingTotalForPicker(row){
  const b=row.booking||row;
  const trip=F.find(t=>String(t.id)===String(b.viaggio_id||b.tratta_id||``));
  const people=Math.max(Number(b.posti||0),0)||(()=>{try{const x=Array.isArray(b.posti_selezionati)?b.posti_selezionati:JSON.parse(b.posti_selezionati||`[]`);return x.length||1}catch{return 1}})();
  return Number(b.totale||0)>0?Number(b.totale):Number(trip?.prezzo||b.prezzo||0)*people;
}
function renderPaymentBookingPicker(){
  if(!paymentBookingPickerList)return;
  const query=String(paymentBookingSearch?.value||``).trim().toLowerCase();
  const rows=L.filter(row=>{
    if(query&&!bookingPickerText(row).includes(query))return false;
    if(pickerType===`Acconto`||pickerType===`Saldo`)return row.summary.residual>0;
    return row.summary.paidNet>0;
  }).slice(0,80);
  paymentBookingPickerList.innerHTML=`<p class="payment-picker__hint">Seleziona una prenotazione: cliente e viaggio vengono collegati automaticamente al movimento.</p>`+
    (rows.length?rows.map(row=>{
      const b=row.booking, summary=row.summary, total=bookingTotalForPicker(row), residual=Math.max(Number(summary.residual)||0,0);
      const people=Number(b.posti||0)||(()=>{try{const x=Array.isArray(b.posti_selezionati)?b.posti_selezionati:JSON.parse(b.posti_selezionati||`[]`);return x.length||1}catch{return 1}})();
      const trip=F.find(t=>String(t.id)===String(b.viaggio_id||b.tratta_id||``));
      const date=trip?.data_partenza||trip?.data||b.data_partenza||b.data_viaggio||null;
      const dateLabel=date?K(date):`Data da verificare`;
      const depositDisabled=pickerType===`Acconto`&&residual<=0;
      const balanceDisabled=pickerType===`Saldo`&&residual<=0;
      return `<article class="payment-picker__item">
        <div class="payment-picker__main">
          <div class="payment-picker__client">${q(b.cliente_nome||b.cliente||`Cliente`)}</div>
          <div class="payment-picker__meta">${q(b.codice||String(b.id||``).slice(0,8))} · ${q(b.telefono||b.cliente_telefono||`Nessun telefono`)}</div>
          <div class="payment-picker__trip">🚌 ${q(row.tripLabel||`Viaggio`)} <span>· ${q(dateLabel)} · ${people} posti</span></div>
          <div class="payment-cash-context"><span class="payment-cash-chip">Incassato ${G(summary.paidNet)}</span><span class="payment-cash-chip ${residual>0?`payment-cash-chip--residual`:`payment-cash-chip--zero`} ">${residual>0?`Residuo ${G(residual)}`:`Pagato`}</span></div>
        </div>
        <div class="payment-picker__numbers"><strong>${G(total)}</strong><span>Totale viaggio</span></div>
        <div class="payment-picker__actions">
          <button type="button" class="picker-select-btn" data-booking-pick="${q(b.id)}" data-picker-type="Acconto" ${depositDisabled?`disabled`:``}>Acconto</button>
          <button type="button" class="picker-select-btn picker-select-btn--secondary" data-booking-pick="${q(b.id)}" data-picker-type="Saldo" ${balanceDisabled?`disabled`:``}>Saldo</button>
          <button type="button" class="picker-select-btn picker-select-btn--open" data-booking-pick="${q(b.id)}" data-picker-type="${q(pickerType)}">Apri</button>
        </div>
      </article>`;
    }).join(``):`<div class="payment-picker__empty">Nessuna prenotazione disponibile per ${q(pickerType)}. Prova con nome, telefono, codice o viaggio.</div>`);
}
function openPaymentBookingPicker(type=`Acconto`){
  pickerType=type;
  if(paymentBookingSearch)paymentBookingSearch.value=``;
  document.querySelectorAll(`.picker-type`).forEach(btn=>btn.classList.toggle(`is-active`,btn.dataset.pickerType===pickerType));
  renderPaymentBookingPicker();
  if(paymentPickerModal){paymentPickerModal.style.display=`flex`;paymentPickerModal.setAttribute(`aria-hidden`,`false`);setTimeout(()=>paymentBookingSearch?.focus(),40)}
}
function closePaymentBookingPicker(){if(paymentPickerModal){paymentPickerModal.style.display=`none`;paymentPickerModal.setAttribute(`aria-hidden`,`true`)}}
function selectPaymentBooking(id,type=pickerType){
  closePaymentBookingPicker();
  Z(id,type);
  setTimeout(()=>D?.focus(),60);
}

function Te(e){if(!e)return;let{booking:t,summary:n,tripLabel:r}=e;const code=t.codice||String(t.id||``).slice(0,8),phone=t.telefono||t.cliente_telefono||`Nessun telefono`,people=Number(t.posti||0)||(()=>{try{const x=Array.isArray(t.posti_selezionati)?t.posti_selezionati:JSON.parse(t.posti_selezionati||`[]`);return x.length||1}catch{return 1}})();v&&(v.textContent=`Pagamenti ${code}`),y&&(y.textContent=`${t.cliente_nome||t.cliente||`Cliente`} • ${r}`),cl&&(cl.textContent=t.cliente_nome||t.cliente||`Cliente`),tr&&(tr.textContent=r||`Viaggio`),bk&&(bk.textContent=`${code} · ${phone} · ${people} ${people===1?`posto`:`posti`}`),selNote&&(selNote.textContent=`Movimento collegato a ${code}`),ci&&(ci.value=t.cliente_id||``),vi&&(vi.value=t.viaggio_id||t.tratta_id||``),b&&(b.textContent=G(n.totalDue)),x&&(x.textContent=G(n.paidNet)),S&&(S.textContent=G(n.residual)),C&&(C.innerHTML=`<span class="status-badge ${ye(n.status)}">${n.status}</span><div class="dg-cassa-reconcile">Totale ${G(n.totalDue)} · Incassato ${G(n.paidNet)} · Residuo ${G(n.residual)}</div>`)}function Z(e,t=null){let n=L.find(t=>String(t.booking.id)===String(e));!n||!_||(z=String(n.booking.id),T&&(T.value=z),Y(),Te(n),we(n),_.style.display=`flex`,t&&O&&(O.value=t,X(t,n.summary)))}function Ee(){_&&(_.style.display=`none`,z=``)}function De(){u.querySelectorAll(`.openLedgerBtn`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.closest(`tr`)?.dataset?.bookingId))}),u.querySelectorAll(`.quickDepositBtn`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.closest(`tr`)?.dataset?.bookingId,`Acconto`))}),u.querySelectorAll(`.quickBalanceBtn`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.closest(`tr`)?.dataset?.bookingId,`Saldo`))}),u.querySelectorAll(`.quickRefundBtn`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.closest(`tr`)?.dataset?.bookingId,`Rimborso`))})}function Q(){let e=J();u.innerHTML=e.length?e.map(xe).join(``):`<tr><td colspan="11" class="empty-state">Nessuna prenotazione corrisponde ai filtri selezionati.</td></tr>`,De(e)}async function $(){P=n(await t.getAll(),[]),F=n(await e.getAll(),[]);let r=await s.getAll();if(r?.success===!1){if(!_e(r.error))throw r.error;I=[],pe||=(U(`Modulo pagamenti non ancora disponibile su Supabase live: vista in sola consultazione prenotazioni.`,`info`),!0)}else I=Array.isArray(r?.data)?r.data:[];ve(),Se(),Q(),_?.style.display===`flex`&&z&&Z(z)}function dgCassaReconcile(e){
