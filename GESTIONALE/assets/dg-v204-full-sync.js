/* DELGROSSO GESTIONALE V204 — FULL SYNC LOCAL-AUTH SAFE */
(function(){
  'use strict';
  if(window.__DG_V204_FULL_SYNC__) return;
  window.__DG_V204_FULL_SYNC__=true;
  const LAST='dg_v204_last_full_sync';
  ...
  function addButton(){
    const el=statusEl();
    if(!el || el.querySelector('.dg-v204-full-sync')) return;
    const b=document.createElement('button');
    b.type='button'; b.className='dg-v204-full-sync';
  ...
  async function run({manual=false}={}){
    if(window.__DG_V204_SYNC_BUSY__) return window.__DG_V204_SYNC_BUSY__;
    const job=(async()=>{
      if(!navigator.onLine) throw new Error('Connessione assente.');
      if(!localAuthOk()) throw new Error('Sessione gestionale non autenticata. Effettua nuovamente il login.');
      addButton();
      ...
      if(manual) console.error('[DG V204 FULL SYNC]',err);
      throw err;
    }).finally(()=>{window.__DG_V204_SYNC_BUSY__=null;});
    window.__DG_V204_SYNC_BUSY__=job;
    return job;
  }
