...
function bind(){
 on('payClientSearch','input',renderClients);
 on('payClientList','click',e=>{const btn=e.target.closest('[data-client-key]');if(!btn)return;selectedClientKey=btn.dataset.clientKey;selectedBookingId='';renderClients();renderTrips();renderSelected();});
 on('payTripList','click',e=>{const btn=e.target.closest('[data-booking-id]');if(!btn)return;selectedBookingId=btn.dataset.bookingId;renderTrips();renderSelected();});
 document.querySelectorAll('[data-pay-type]').forEach(btn=>btn.addEventListener('click',()=>setType(btn.dataset.payType)));
 on('paySave','click',save);
 on('payRefresh','click',load);
 on('payOpenBookings','click',()=>location.href='./prenotazioni.html');
 on('payBack','click',()=>history.back());
 on('payAmount','input',()=>{});
 on('payStatusBar','click',()=>{});
}
