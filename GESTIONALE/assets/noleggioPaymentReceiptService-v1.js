...
export async function issueNoleggioPaymentReceipt(payment,rental,totals={},options={}){
  ...
  return call({payment:{...payment,receipt_number:built.receiptNumber},rental,totals,pdfBase64:await blob64(built.blob),sendEmail:options.sendEmail!==false}).then(d=>({...built,...d}));
}
