/* DELGROSSO VIAGGI — V50 STUDIO SHELL
   Navigation/presentation only. Business logic remains in existing modules. */
(() => {
  'use strict';
  if (window.__DG_SHELL_V50__) return;
  window.__DG_SHELL_V50__ = true;
  ...
  const ROUTES = {
    ...
    checkin:'./checkin.html', statistiche:'./statistiche.html',
    centrooperativo:'./centro-operativo.html', impostazioni:'./impostazioni.html'
  };
  ...
  const NAV = [
    ...
    ['CHECK-IN','checkin','⌗'], ['Statistiche','statistiche','▥'],
    ['Centro Operativo','centrooperativo','◉'], ['Impostazioni','impostazioni','⚙']
  ];
  ...
  const injectStudioCss = () => {
    if (document.querySelector('link[data-dg-studio-v50]')) return;
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = './assets/dg-studio-v50.css?v=5001';
    link.dataset.dgStudioV50 = '1';
  ...
  function build() {
    ...
    injectStudioCss();
    removeLegacyShells();
    document.body.classList.add('dg-v50-ready');
    ...
    shell.innerHTML = `
      ...
      <header class="dg-v41-header" role="banner">
        <div class="dg-v41-heading">
          <span>DELGROSSO VIAGGI · TRAVEL MANAGEMENT</span>
          <strong>${esc(TITLES[key] || 'Gestionale')}</strong>
        </div>
        <div class="dg-v41-status"><span class="dg-v41-dot"></span><span>Operativo · V50 STUDIO</span></div>
      </header>
      <aside id="dg-v41-drawer" class="dg-v41-drawer" aria-label="Navigazione Gestionale" aria-hidden="true">
        <div class="dg-v41-brand">
          ...
          <div class="dg-v41-brand-copy"><small>TRAVEL MANAGEMENT</small><strong>DELGROSSO VIAGGI</strong><em>Gestionale · Studio V50</em></div>
        ...
        <div class="dg-v41-account">
          <div class="dg-v41-avatar">${esc(String(u.nome||u.username||'G').trim().charAt(0).toUpperCase())}</div>
          <div><strong>${esc(u.nome||u.username||'Gestionale')}</strong><span>${esc(u.ruolo||'Operatore')}</span></div>
        </div>
    ...
    shell.querySelectorAll('.dg-v41-nav a').forEach(a => a.addEventListener('click', () => {
      if (window.matchMedia('(max-width:1050px)').matches) setOpen(false);
    }));
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape' && document.body.classList.contains('dg-v41-open')) setOpen(false);
    });

    shell.querySelector('.dg-v41-logout').addEventListener('click', () => {
      try { localStorage.removeItem('dg_session'); }
    ...
    logo?.addEventListener('error', () => {
      ...
      shell.querySelector('.dg-v41-logo span').style.display='block';
    }, {once:true});
  }

  const boot = () => { try { build(); } catch (e) { console.error('[DG V50 shell]', e); } };
