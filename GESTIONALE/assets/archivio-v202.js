...
async function restArchive(){
  ...
  const noleggioDocs=(nbRes.ok?nb:[]).map(x=>({
...
function normalize(documents){
  return (documents||[]).map(d=>{
    ...
    return {
      ...
      number:d.id_prenotazione||d.numero_documento||((kind==='booking')?d.confirmation_number:d.receipt_number)||'Da generare',
      customer:customer(d),
      trip:trip(d),
...
function stats(){
  ...
  const booking=rows.filter(x=>x.kind==='booking').length;
  const acconto=rows.filter(x=>x.kind==='acconto').length;
  const saldo=rows.filter(x=>x.kind==='saldo').length;
  const missing=rows.filter(x=>!x.path).length;
  $('#stat-total').textContent=total; $('#stat-booking').textContent=booking; $('#stat-acconto').textContent=acconto; $('#stat-saldo').textContent=saldo;
  const box=$('#archive-missing-count');
  const live=$('#archive-live-summary');
  if(live) live.textContent=`Prenotazioni/documenti: ${booking} · Acconti: ${acconto} · Saldi: ${saldo} · Mancanti: ${missing}`;
  if(box) box.textContent=missing?`⚠ ${missing} documenti da generare/archiviare`:'✓ Tutti i documenti archiviati';
}
...
function render(){
  ...
  body.innerHTML=filtered.map(r=>`<tr>
    ...
    <td><div class="archive-actions">
      ${r.path?'':'<button class="archive-btn archive-missing-action" data-action="archive" data-id="'+esc(r.id)+'">Genera e archivia</button>'}<button class="archive-btn primary" data-action="open" data-id="${esc(r.id)}" ${r.path?'':'disabled'}>Apri PDF</button>
